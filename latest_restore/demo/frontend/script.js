...
async function analyzePro() {
    ...
    try {
        ...
        addLog("Análisis completado. Reporte listo para revisión.");
    ...
    if (data.enrichment && enrichContainer) {
        ...
        data.enrichment.slice(0, 50).forEach(item => {
            let pLinks = item.Evidences && item.Evidences.length > 0 
                ? item.Evidences.slice(0, 10).map(id => `<a href="https://pubmed.ncbi.nlm.nih.gov/${id}" target="_blank" style="color:var(--accent); text-decoration:none; margin-right:5px;">[PMID:${id}]</a>`).join("") 
                : 'Verificado';
            html += `<tr><td><strong>${item.Term}</strong><br><small>${item.ScientificDesc}</small></td><td><span class="status-badge" style="background:#30363d; color:var(--text-dim);">${item.Source}</span></td><td>${item.Pval.toExponential(2)}</td><td><div style="display:flex; flex-wrap:wrap; gap:2px;">${pLinks}</div></td></tr>`;
        });
        html += "</tbody></table>";
...
function populateReport() {
    ...
    // Mostrar hasta 30 genes en la tabla del reporte
...
function exportToPDF() {
    ...
    // Capturamos el elemento directamente. Como es el mismo que el usuario edita, se guardan los cambios.
    ...
    html2pdf().set(opt).from(reportElement).save().then(() => { 
        addLog("Informe descargado con éxito.");
    });
}
