...
async def run_pipeline(request: PipelineRequest):
            ...
            for label, lib in libs.items():
                ...
                if not res.empty:
                    ...
                    for _, row in top.iterrows():
                        evidence = await get_pubmed_details(row['Term'], request.years, client)
    ...
    exec_summary = f"El análisis identificó {len(common_all)} dianas consenso sometidas a regulación simultánea. Se observa una convergencia epigenética significativa hacia fenotipos complejos."

    return {
        "common_genes": common_all, "venn_plot": venn_b64, "ppi_plot": ppi_b64,
        "volcano_plot": volcano_b64, "enrichment": enrichment_data,
        "executive_summary": exec_summary, "logs": logs
    }
